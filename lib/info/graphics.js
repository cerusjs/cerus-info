...
class graphics {
	constructor(cerus) {
		return cerus.promise(function(event) {
			si.graphics(function(graphics_) {
				this._controllers = graphics_.controllers.map(function(controller_) {
					return new controller(controller_);
				});

				this._displays = graphics_.displays.map(function(display_) {
					return new display(display_);
				});
